...
async function handleSaveCard({ userId }) {
    ...
    // Используем VERCEL_URL или fallback на production домен
    const baseUrl = process.env.VERCEL_URL
        ? `https://${process.env.VERCEL_URL}`
        : 'https://prizmaaaa.vercel.app';
    const returnUrl = `${baseUrl}/profile.html?card_saved=true`;
    ...
    const paymentData = {
        ...
        confirmation: { type: 'redirect', return_url: returnUrl }, // Redirect back to profile
        receipt: {
